...
// ===== Checkout -> create order & redirect to order.html + WhatsApp (Firestore) =====
(function () {
  'use strict';
  ...
  // CHECKOUT: simpan order ke Firestore + local fallback + buka WA + redirect ke Orders
  document.addEventListener('click', async function(e){
    ...
    // prepare Firestore payload similar ke cekout.js
    ...
    // try save to Firestore (non-blocking on UI)
    try {
      ...
      // optionally store docRef.id into local copy
      order._firestoreId = docRef.id;
    } catch (err) {
      console.error("Gagal menyimpan order ke Firestore:", err);
      // continue — still store local copy for fallback
    }

    // save local copy for offline fallback + admin old UI (optional)
